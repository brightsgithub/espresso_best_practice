...
class Screen5ActivityHybridTest {
    ...
    fun verifyScreenElementsAndNavigation() {
        ...
        onView(withId(R.id.titleTextView5)).check(matches(withText("Screen $num")))

        // Check that the subtext text view has the correct text
        onView(withId(R.id.subtextTextView5)).check(matches(withText("Some random subtext for screen $num")))

        // Check that the next button has the correct text
        onView(withId(R.id.nextButton5)).check(matches(withText("submit")))

        // Click the next button and check that the Screen3Activity is launched
        onView(withId(R.id.nextButton5)).perform(click())
        onView(withId(R.id.titleTextView6)).check(matches(isDisplayed()))
    ...
    private fun launchMyScreen() {
        TestUtils.launchScreen(
            Screen5Activity.getLaunchingIntent(
