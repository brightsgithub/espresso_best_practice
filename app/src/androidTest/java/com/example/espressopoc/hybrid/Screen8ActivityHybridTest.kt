...
class Screen8ActivityHybridTest {
    ...
    fun verifyScreenElementsAndNavigation() {
        ...
        onView(withId(R.id.titleTextView8)).check(matches(withText("Screen $num")))

        // Check that the subtext text view has the correct text
        onView(withId(R.id.subtextTextView8)).check(matches(withText("Some random subtext for screen $num")))

        // Check that the next button has the correct text
        onView(withId(R.id.nextButton8)).check(matches(withText("submit")))

        // Click the next button and check that the Screen9Activity is launched
        onView(withId(R.id.nextButton8)).perform(click())
        onView(withId(R.id.titleTextView9)).check(matches(isDisplayed()))

        // Perform press back button action
        Espresso.pressBack()

        // Check if the current activity is Screen8Activity
        onView(withId(R.id.titleTextView8)).check(matches(withText("Screen $num")))
    }

    private fun launchMyScreen() {
        TestUtils.launchScreen(
            Screen8Activity.getLaunchingIntent(
